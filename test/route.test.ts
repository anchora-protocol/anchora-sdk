import { Token, WROSE, ChainId, Pair, TokenAmount, Route, ROSE } from '../src'

describe('Route', () => {
  const token0 = new Token(ChainId.MAINNET, '0x0000000000000000000000000000000000000001', 18, 't0')
  const token1 = new Token(ChainId.MAINNET, '0x0000000000000000000000000000000000000002', 18, 't1')
  const weth = WROSE[ChainId.MAINNET]
  ...
  it('supports ether input', () => {
    const route = new Route([pair_0_weth], ROSE)
    expect(route.pairs).toEqual([pair_0_weth])
    expect(route.input).toEqual(ROSE)
    expect(route.output).toEqual(token0)
  })

  it('supports ether output', () => {
    const route = new Route([pair_0_weth], token0, ROSE)
    expect(route.pairs).toEqual([pair_0_weth])
    expect(route.input).toEqual(token0)
    expect(route.output).toEqual(ROSE)
  })
})
