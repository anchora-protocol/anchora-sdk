...
describe('Trade', () => {
  ...
  const pair_weth_0 = new Pair(
    new TokenAmount(WROSE[ChainId.MAINNET], JSBI.BigInt(1000)),
  ...
  it('can be constructed with ROSE as input', () => {
    const trade = new Trade(
      new Route([pair_weth_0], ROSE),
      CurrencyAmount.ether(JSBI.BigInt(100)),
      TradeType.EXACT_INPUT
    )
    expect(trade.inputAmount.currency).toEqual(ROSE)
    expect(trade.outputAmount.currency).toEqual(token0)
  })
  it('can be constructed with ROSE as input for exact output', () => {
    const trade = new Trade(
      new Route([pair_weth_0], ROSE, token0),
  ...
  it('can be constructed with ROSE as output', () => {
    const trade = new Trade(
      new Route([pair_weth_0], token0, ROSE),
      CurrencyAmount.ether(JSBI.BigInt(100)),
      TradeType.EXACT_OUTPUT
    )
    expect(trade.inputAmount.currency).toEqual(token0)
    expect(trade.outputAmount.currency).toEqual(ROSE)
  })
  it('can be constructed with ROSE as output for exact input', () => {
    const trade = new Trade(
      new Route([pair_weth_0], token0, ROSE),
      new TokenAmount(token0, JSBI.BigInt(100)),
      TradeType.EXACT_INPUT
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for ROSE currency input', () => {
      const result = Trade.bestTradeExactIn(
      ...
      expect(result).toHaveLength(2)
      expect(result[0].inputAmount.currency).toEqual(ROSE)
      expect(result[0].route.path).toEqual([WROSE[ChainId.MAINNET], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(ROSE)
      expect(result[1].route.path).toEqual([WROSE[ChainId.MAINNET], token0, token3])
      expect(result[1].outputAmount.currency).toEqual(token3)
    })
    it('works for ROSE currency output', () => {
      const result = Trade.bestTradeExactIn(
        [pair_weth_0, pair_0_1, pair_0_3, pair_1_3],
      ...
      expect(result[0].route.path).toEqual([token3, token0, WROSE[ChainId.MAINNET]])
      expect(result[0].outputAmount.currency).toEqual(ROSE)
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].route.path).toEqual([token3, token1, token0, WROSE[ChainId.MAINNET]])
      expect(result[1].outputAmount.currency).toEqual(ROSE)
    })
  })
