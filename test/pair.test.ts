import { ChainId, Token, Pair, TokenAmount, WROSE, Price } from '../src'
...
describe('Pair', () => {
  ...
  describe('constructor', () => {
    it('cannot be used for tokens on different chains', () => {
      expect(() => new Pair(new TokenAmount(USDC, '100'), new TokenAmount(WROSE[ChainId.TESTNET], '100'))).toThrow(
        'CHAIN_IDS'
      )
    })
  })
  ...
  describe('#priceOf', () => {
    ...
    it('throws if invalid token', () => {
      expect(() => pair.priceOf(WROSE[ChainId.MAINNET])).toThrow('TOKEN')
    })
  })
  ...
  describe('#reserveOf', () => {
    ...
    it('throws if not in the pair', () => {
      expect(() =>
        new Pair(new TokenAmount(DAI, '101'), new TokenAmount(USDC, '100')).reserveOf(WROSE[ChainId.MAINNET])
      ).toThrow('TOKEN')
    })
  })
  ...
  describe('#involvesToken', () => {
    ...
    expect(
      new Pair(new TokenAmount(USDC, '100'), new TokenAmount(DAI, '100')).involvesToken(WROSE[ChainId.MAINNET])
    ).toEqual(false)
  })
})
