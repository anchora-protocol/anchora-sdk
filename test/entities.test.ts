...
import { ChainId, WROSE as _WROSE, TradeType, Rounding, Token, TokenAmount, Pair, Route, Trade } from '../src'
...
const WROSE = _WROSE[ChainId.TESTNET]
    ...
    describe(`decimals permutation: ${decimals}`, () => {
      ...
      it('Pair', () => {
        pairs = [
          ...
          new Pair(
            new TokenAmount(tokens[2], decimalize(1, tokens[2].decimals)),
            new TokenAmount(WROSE, decimalize(1234, WROSE.decimals))
          )
        ]
      })
      ...
      it('Route', () => {
        ...
        expect(route.path).toEqual(tokens.concat([WROSE]))
        expect(route.input).toEqual(tokens[0])
        expect(route.output).toEqual(WROSE)
      })
      ...
      describe('Trade', () => {
          ...
          route = new Route(
            [
              new Pair(
                ...
                new TokenAmount(WROSE, decimalize(10, WROSE.decimals))
              )
            ],
            tokens[1]
          )
          const inputAmount = new TokenAmount(tokens[1], decimalize(1, tokens[1].decimals))
          const expectedOutputAmount = new TokenAmount(WROSE, '1663192997082117548')
        ...
        it('TradeType.EXACT_OUTPUT', () => {
          const outputAmount = new TokenAmount(WROSE, '1663192997082117548')
      ...
      it('TokenAmount', () => {
        const amount = new TokenAmount(WROSE, '1234567000000000000000')
